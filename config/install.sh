...
if [ x"$ARCH" = "xx86" -a x"$OPSYS" = "xlinux" ] ; then
  case `uname -m` in
	...
	complain "$this: !!! SML/NJ requires support for 32-bit executables. On Debian Linux, you may need to apt-get install gcc-multilib g++-multilib ia32-libs.  On RH Fedora, try: yum install glibc-devel.i686"
      else
	rm -f /tmp/$tmpFile /tmp/$tmpFile.c
      fi
    ;;
