 ...
 *
 * This phase takes a cluster with pseudo amd64 fp instructions, performs
...
functor AMD64FP
   (structure AMD64Instr  : AMD64INSTR
    structure AMD64Props  : INSN_PROPERTIES 
			      where I = AMD64Instr
...
struct
   ...
   val name = "AMD64 floating point rewrite"

   val debugOn = MLRiscControl.mkFlag ("amd64-fp-debug", "amd64 fp debug mode")
   val traceOn = MLRiscControl.mkFlag ("amd64-fp-trace", "amd64 fp trace mode")

   fun error msg = MLRiscErrorMsg.error("AMD64FP",msg)
   ...
   (*-----------------------------------------------------------------------
    * A stack datatype that mimics the amd64 floating point stack
   ...
   *)
   ...
   let
       ...
       let val (stackIn, stack, code) = shuffleIn(blknum, block, liveIn)
		...
		| I.INSTR instr => amd64trans(instr)
