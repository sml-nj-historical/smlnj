...
local 
  ...
  structure FL = FLINT (* which depends on Lty for basic types *)

  open Lty
  open LtyStructure
  open LtyUtil

  fun bug msg = ErrorMsg.impossible("LtyMisc: "^msg)
  val say = Control.Print.say

  (** common utility functions *)
  val tk_inj = LT.tk_inj
  val tk_out = LT.tk_out

  val tc_inj = LT.tc_inj
  val tc_out = LN.tc_out_nm (* out projection with normalization *)
 
  val lt_inj = LT.lt_inj
  val lt_out = LN.lt_out_nm (* out projection with normalization *)

  val tcc_env = LN.tcc_env
  val ltc_env = LN.ltc_env
  val tc_whnm = LN.tc_whnm
  val lt_whnm = LN.lt_whnm
  val tc_norm = LN.tc_norm
  val lt_norm = LN.lt_norm
...
(* not used
val (_,_,teKindChk) = LtyKindChk.tcteKindCheckGen ()
*)
...
fun lt_inst_chk_gen() = let
    ...
    fun lt_inst_chk (lt : lty, ts : LT.tyc list, kenv : tkindEnv) = 
        let val nt = lt_whnm lt
        in (case ((* lt_outX *) lt_out nt, ts)
              of (LT.LT_POLY(ks, b), ts) => 
...
fun tc_upd_prim tc = 
  let fun h(LT.TC_PRIM pt) = 
        ...
        | h(LT.TC_SUM tcs) = 
            let fun g (a::r) = if LN.tc_eqv(a, tcc_unit) then g r else false
                  | g [] = true
           ...
           | LT.TC_ARROW (LT.FF_VAR(b1,b2), ts1, ts2) =>  
               let val nts1 =    (* too specific ! *)                       
                     (case ts1 of [t11,t12] => [w t11, w t12] 
                                | _ => [w (LN.tc_autotuple ts1)])
                   val nts2 = [w (LN.tc_autotuple ts2)]
      ...
      fun tc_umap (u, w) t =
        (case (tc_out t)
          of (LT.TC_VAR _ | LT.TC_NVAR _ | LT.TC_PRIM _) => t
           ...
           | LT.TC_TOKEN (k, t) => 
               if LT.token_eq(k, LN.wrap_token) then 
...
fun tc_nvar_subst_gen() = let
    ...
    fun tc_nvar_subst subst = let
        ...
        (case intersectionNonEmpty(subst, LN.tc_nvars tyc) of
...
fun lt_nvar_subst_gen() = let
    ...
    fun lt_nvar_subst subst = let
        ...
        (case intersectionNonEmpty(subst, LN.lt_nvars lty) of
...
fun tc_nvar_cvt_gen() = let
    ...
    fun tc_nvar_cvt (tvoffs:tvoffs) d tyc = 
        ((* Stats.addStat s_iter 1; *)
         ...
         (* Stats.addStat s_nvars (length (LN.tc_nvars tyc)); *)
        (* check if substitution overlaps with free vars list *)
        case intersect(tvoffs, LN.tc_nvars tyc) of
...
fun lt_nvar_cvt_gen() = let
    ...
    fun lt_nvar_cvt tvoffs d lty = 
        (* check if substitution overlaps with free vars list *)
        case intersect(tvoffs, LN.lt_nvars lty) of
            [] => lty                (* nothing to cvt *)
          | tvoffs => 
    let
        (* encode the lty and depth info using LT_ENV
