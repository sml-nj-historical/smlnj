...
(* 
 * A SHARE constraint is essentially a directed arc indicating that two
	  ...
	  of CONST_ENT strEnt => (strEnt,failuresSoFar)  (* already visited *)
	   ...
	   | GENERATE_ENT closedDef =>
		...
		fun instToTyc(ref(INST tycon),_) = tycon 
		  ...
		  | instToTyc(r as ref(NOTINST tycon), entEnv) =
		    ...
		    in
		     case tycon
			   ...
			   (let val tf = T.TYFUN{arity=arity, 
			        ...
			        val tc = T.DEFtyc{tyfun=tf, strict=strict, 
					    stamp=mkStamp(),
					    path=IP.append(rpath,path)}
			...
			| T.GENtyc {stamp,arity,eq,path,kind,...} =>
			  (case kind
			     of T.DATATYPE {index,freetycs,stamps,
                                                family, root} =>
			       (let
		...
		fun instToEntity (sym,slot,entEnv,failuresSoFar:int)
		    ...
		    case !slot
		     of inst as (FinalStr _) =>
			  let val (strEntity, failuresSoFar') =
			          instToStr'(inst, entEnv, IP.extend(rpath,sym),
					     failuresSoFar)
			   in (STRent strEntity, failuresSoFar')
			  end
		  ...
		  * in checkTycBinding in SigMatch.
		  * Fixes bugs 1364 and 1432. [DBM]
		  *)
		...
		fun mkEntEnv (entEnv_fc) = 
		    foldl (fn ((sym,spec),(env,failCount)) => 
 	        ...
 	        val (entEnv',failCount) = 
                    ...
                    else (* a potentiatlly open signature with external 
			  * tycon references in the form of entpaths *)
                      let val _ = debugmsg "mkEntEnv: not closed";
                          val baseEntEnv = 
                              MARKeenv{stamp = mkStamp(),
				       env = entEnv,
				       stub = NONE}
                          val (ee, fc) = mkEntEnv(baseEntEnt,failuresSoFar)
                       in (ee, fc)
                      end
