...
functor SIRedBlackSetFn (K : ORD_KEY) :> SI_ORD_SET where Key = K =
  ...
  (* Return the least item in the set, if the set is not empty *)
    fun least(SET(_,t)) = 
	let fun find E = NONE
	      | find(T(_,E,a,_)) = SOME a
	      | find(T(_,t,_,_)) = find t
	in find t
	end

  (* Return the greatest item in the set, if the set is not empty *)
    fun greatest(SET(_,t)) = 
	let fun find E = NONE
	      | find(T(_,_,a,E)) = SOME a
	      | find(T(_,_,_,t)) = find t
	in find t
	end

  (* val next : set * item -> item option *)
  (* Return the least item in the set greater or equal to the item, if one exists.
     Note that the item need not be a member of the set. *)
    fun nextOrEqual (SET(_,t), k) =
	let fun find E = NONE
	      | find (T(_,a,y,b)) =
		  (case K.compare(k,y)
		    of GREATER => find b
  ...
  (* Return the greatest item in the set less than the item, if one exists.
     Note that the item need not be a member of the set. *)
    fun previous (SET(_,t), k) =
	let fun find E = NONE  (* empty set *)
	      | find (T(_,a,y,b)) =
		  (case K.compare(k,y)
		    of (LESS | EQUAL) => find a
		     | GREATER => 
		       (case find b
			 of NONE => SOME y
			  | r => r))
