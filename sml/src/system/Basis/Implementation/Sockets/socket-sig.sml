 ...
 *
 * extracted from socket.mldoc (v. 1.0; 1998-06-05)
 *)
...
signature SOCKET =
  sig
    ...
    val sendVec : ('af, active stream) sock * Word8Vector.vector buf -> int
    val sendArr : ('af, active stream) sock * Word8Array.array buf -> int
    val sendVec' : ('af, active stream) sock
                     * Word8Vector.vector buf
                     * out_flags -> int
    val sendArr' : ('af, active stream) sock * Word8Array.array buf * out_flags
                     -> int
    val sendVecTo : ('af, dgram) sock * 'af sock_addr * Word8Vector.vector buf
                      -> int
    val sendArrTo : ('af, dgram) sock * 'af sock_addr * Word8Array.array buf
                      -> int
    ...
    val recvArr : ('af, active stream) sock * Word8Array.array buf -> int
    val recvVec' : ('af, active stream) sock * int * in_flags
                     -> Word8Vector.vector
    val recvArr' : ('af, active stream) sock * Word8Array.array buf * in_flags
                     -> int
