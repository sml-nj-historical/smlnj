...
placing appropriately-named plug-in libraries into a global tool
repository.  Of course, it will also require a corresponding
re-configuration of the anchor environment.  In other words, for
general-purpose tools that are installed in some central place,
corresponding tool description files {\tt \$/}$c${\tt -tool.cm} and
{\tt \$/}$s${\tt -ext.cm} should be registered using the path anchor
mechanism.  If this is done, actual description files can be placed in
arbitrary locations.
...
tool} lasts until the end of the current description file.  Moreover,
the effect is {\em restricted} to the current description file.  This
means that other description files that also want to use class {\tt
bar} will have to have their own {\tt tool} entry.

\noindent{\bf Important:} Registration of classes and classifiers
happens by link-time side-effect.  If a class or a classifier has been
