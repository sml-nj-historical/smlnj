...
{\it\small (for SML/NJ version 110.27 and later)} \\
User Manual}
...
As far as CM's core tool mechanism is concerned, tool parameters are
uninterpreted---they are just list of strings.  Each rule is
responsible for assigning meaning to its own parameters.

The {\tt sml} class accepts one parameter which must be either the
word {\tt shared} or the word {\tt private}.  If {\tt shared} is
...
CM.autoload}.  As explained earlier (Section~\ref{sec:sharing}), the
{\tt private} annotation means that dynamic state cannot be shared
across such calls to {\tt CM.make} or {\tt CM.autoload}.
...
that the member itself is the source.  The {\tt options=} parameter
lists the tool parameters to be used for that target.  The command
line itself is given by the remaining non-keyword parameters.  Here, a
single {\bf \%s} is replaced by the source file name, and a single {\bf
\%t} is replaced by the target file name.
...
target class can be given by writing a {\tt class=} parameter.

If the member name is written using {\em native path syntax}, then the
source- or target-name in the parameter list is also considered to be
...
path name (although in this case it does not make a difference).  Had
we used the {\tt target=} version from above, it would {\em not} be
...
Any tool parameters beyond the initial {\tt class=} and {\tt options=}
are given as additional command line arguments to {\tt make}.  The
target name is always the last command line argument.
...
Cascading Shell- and Make-tools is tricky but possible.  Here is an
...
suffix string, where the second component optionally specifies the
member class name of the corresponding derived file, and where the
...
functions is {\tt string list option}.)
...
\end{enumerate}
\item[dflopts] an optional list of strings which is used for
