...
foo-lib.cm itself or other groups thereof may list foo-utils.cm as one
of their members)
...
foo-utils.cm}.  In this case it could be that {\tt foo-utils.cm} is
mentioned indirectly via a chain of other components of {\tt
foo-lib.cm}.  The other possibility is that it is not mentioned at all
(in which case CM would never know about it and not complain).
...
described above) can always be re-written as {\tt \$foo/foo/bar/baz}.
...
error.  The default ({\tt true}, i.e, implicit anchors are being
accepted) can be overridden at startup by the environment variable
{\tt CM\_IMPLICIT\_ANCHORS}.
...
description file (see Section~\ref{sec:classes}).  In other words, to
mark an ML file as {\em private}, follow the file name with the word
{\tt private} in parentheses.  For {\em shared} ML files, replace {\tt
private} with {\tt shared}.
...
via {\tt CM.make} or {\tt CM.load\_plugin}.  ({\tt CM.autoload} is not
enough because of its lazy nature which prevents the required
...
the most recent going by the same name ``CM''~\cite{blume95:cm}, while
...
lines (lines starting with {\bf \#}) or following one of the SML
keywords.
...
using environment variables $v_s$ at startup time, i.e,
when the {\tt sml} command is invoked.  Each such parameter has a
default setting.  Default settings are determined at bootstrap time,
i.e., the time when the heap image for SML/NJ's interactive system is
...
At bootstrap time, it is possible to adjust defaults by using a
...
{\tt CM\_$X$\_DEFAULT} variables at bootstrap time, there are two
columns with default values in the following table.  The value labeled
{\em fallback} is the one that would have been used had there been no
environment variable at bootrap time, the one labeled {\em default} is
the one the user will actually see.

To save space, the table only lists the {\em stem} of each variable
name.  If the stem is $X$, then $v_s$ has the form {\tt CM\_$X$} and
$v_b$ has the form {\tt CM\_$X$\_DEFAULT}.  For example, CM's
verbosity can be adjusted using {\tt CM\_VERBOSE} and the default can
be set using {\tt CM\_VERBOSE\_DEFAULT}.
...
name & description & installed & autoloaded \\
\hline\hline
{\tt \$/basis.cm} & Standard Basis Library & always & yes \\
\hline\hline
...
architecture & always & yes \\
\hline\hline
{\tt \$smlnj/cm.cm} \newline
{\tt \$smlnj/cm/full.cm} & compilation manager & always & yes \\
\hline
