(* pp.sml *)
(* Konrad Slind, <Konrad.Slind@cl.cam.ac.uk>, 5 May 1997 *)
...
(*---------------------------------------------------------------------------
   ...
   Added two new internal fields to the ppstream datatype: "panic_column" 
   and "reset_column". When a block is "begun" at or past the panic
   column, the block will instead begin at the reset column. This will
   help control "runaway" prettyprinting. Currently, the panic column is
   at 2/3 of the linewidth, and the reset column is at 1/3 the linewidth.
...
fun error msg = raise PP_FAIL ("PP FAILURE: " ^ msg)
...
with
  ...
  fun top (Istack stk) =
      case !stk
        of nil => error "top: badly formed block"
	 | x::_ => x
  fun push (x,(Istack stk)) = stk := x::(!stk)
  fun pop (Istack stk) =
      case !stk
	of nil => error "pop: badly formed block"
	 | _::rest => stk := rest
end
...
datatype ppstream = 
  PPS of
     {consumer : string -> unit,
      ...
      panic_column : int,      (* When is space_left too small *)
      reset_column : int       (* New start col. when space_left too small *)}
...
fun mk_ppstream {consumer,linewidth,flush} =
    if (linewidth<5)
    then error "linewidth too small"
	  ...
	  in PPS{consumer = consumer,
		 ...
		 left_sum = ref 0, right_sum = ref 0,
                 panic_column = (linewidth * 2) div 3,
                 reset_column = linewidth div 3}
	 end
...
local val space = #" "
      ...
      fun nspaces n = Vector.sub(space_table,n) handle Subscript 
          => if n < 0 then ""
             else let val n2 = n div 2
                      val n2_spaces = nspaces n2
                      val extra = if (n = (2*n2)) then "" else " "
                  in String.concat [n2_spaces, n2_spaces, extra] end
in
...
(*---------------------------------------------------------------------------
   Print the first member of a contiguous sequence of Begins. If there
 ...
 ----------------------------------------------------------------------------*)
fun print_BB (_,{Pblocks = ref [], Ublocks = ref []}) = error"print_BB"
  ...
  | print_BB (PPS{the_indent_stack, linewidth, space_left=ref sp_left,...},
              {Ublocks,...}) = let 
      fun pr_end_Ublock[{How_to_indent=CONSISTENT,Block_size,Block_offset}] l =
          (push ((if (!Block_size > sp_left)  
                  then ONE_PER_LINE (linewidth - (sp_left - Block_offset))
                  else FITS),  the_indent_stack);
           rev l)
...
(*---------------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------------*)
fun print_E (_,{Pend = ref 0, Uend = ref 0}) = error "print_E"
  | print_E (istack,{Pend, ...}) =
...
fun print_token(PPS{consumer,space_left,...}, S{String,Length}) = 
      (consumer String; space_left := (!space_left) - Length)
  | print_token(ppstrm,BB b) = print_BB(ppstrm,b)
  | print_token(PPS{the_indent_stack,...},E e) = 
      print_E (the_indent_stack,e)
  | print_token (PPS{the_indent_stack,space_left,consumer,
                     linewidth,panic_column, reset_column, ...},
     ...
     (case (top the_indent_stack)
        of FITS =>
         ...
         | (ONE_PER_LINE cursor) => 
             let val cbo = cursor + Break_offset
                 val new_cursor = if cbo > panic_column then reset_column
                                  else cbo
	     ...
	     then let val cbo = cursor + Break_offset
                      val new_cursor = if cbo > panic_column then reset_column
                                       else cbo
		   in space_left := linewidth - new_cursor;
		      cr_indent(consumer,new_cursor)
...
fun clear_ppstream(PPS{the_token_buffer, the_delim_stack, 
 ...
 let val buf_size = 3*linewidth
     fun set i = if (i = buf_size) then ()
                 else(update(the_token_buffer,i,initial_token_value); set(i+1))
 in set 0;
...
let val NEG = ~1
    ...
    fun token_size (S{Length, ...}) = Length
      | token_size (BB b) =
        (case b 
         of {Pblocks = ref[], Ublocks = ref[]} => error "BB_size"
          | {Pblocks as ref(_::_),Ublocks=ref[]} => POS
	  | {Ublocks, ...} => last_size (!Ublocks))
      | token_size (E{Pend=ref 0, Uend=ref 0}) = error"token_size.E"
    ...
    fun loop (instr) =
        ...
        then ()
	      ...
	      inc_left_sum instr;
	      if (pointers_coincide ppstrm) then ()
              else (* increment left index *)

    (*-----------------------------------------------------------------------
       When this arm is entered, we know that the left_index has not yet
      ...
      -----------------------------------------------------------------------*)
              (case (the_token_buffer sub (!left_index))
               of (BB {Pblocks = ref [], Ublocks = ref []}) => 
                  (update(the_token_buffer, !left_index,initial_token_value);
                   ++left_index)
                | (BB _) => ()
		  ...
		  (update(the_token_buffer,!left_index, initial_token_value); 
                   ++left_index)
                | (E _) => ()
                | _ => ++left_index;loop (the_token_buffer sub (!left_index))))
     in loop instr
    end
...
local
  fun check_delim_stack(PPS{the_token_buffer,the_delim_stack,right_sum,...}) =
    let fun check k =
         if (delim_stack_is_empty the_delim_stack) then ()
         else case(the_token_buffer sub (top_delim_stack the_delim_stack))
	      of BB{Ublocks as ref ((b as {Block_size, ...})::rst), Pblocks} 
                  => if (k>0)
                     then (Block_size := !right_sum + !Block_size;
			   Pblocks := b :: (!Pblocks);
                           Ublocks := rst;
                           if (List.length rst = 0)
                           then pop_delim_stack the_delim_stack else ();
                           check(k-1))
                     else ()
	      | E{Pend,Uend}
                  => (Pend := (!Pend) + (!Uend);
              ...
              | BR{Distance_to_next_break, ...}
                 => (Distance_to_next_break :=  !right_sum + 
                                                !Distance_to_next_break;
                     pop_delim_stack the_delim_stack;
                     if (k>0) then check k else ())
              | _ => error "check_delim_stack.catchall"
    in check 0
    end
in
...
(*---------------------------------------------------------------------------
  Derived forms for making ppstreams that observe system parameters. 

  fun stdOut_consumer() = 
    let val {say,flush} = !Control.Print.out
    in {consumer=say, flush=flush, 
        linewidth = !Control.Print.linewidth}
    end;

   val stdOut_ppstream = mk_ppstream o stdOut_consumer;
