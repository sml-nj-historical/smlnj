...
(*	  val nodes = (mkRules res) :: nodes' *)
    ...
    fun output (spec, fname) = let
	  ...
	  fun outMach (LO.Machine {label, rules, states}) = let
		val res = Vector.map #1 rules
		val outfn = fname ^ "-" ^ label ^ ".dot"
		val out = TextIO.openOut outfn
		val graph = mkGraph (res, states)
val _ = print (" writing " ^ outfn ^ "\n")
		in 
		  writeGraph (out, graph)
		  before TextIO.closeOut out
		end
          in 
	    List.app outMach machines
          end
