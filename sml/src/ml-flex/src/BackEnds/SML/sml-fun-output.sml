   ...
   *)
    fun mkTrans ([], _) = raise Fail "BUG: SMLFunOutput, alphabet not covered"
	  ...
	  fun split (ls, 0, l1) = (List.rev l1, ls)
	    ...
	    | split ([], _, _) = raise Fail "split: empty"
    ...
    fun mkState ruleToAction (s, k) = let
          val LO.State {id, label, final, next} = s
	  fun addMatch (ruleIdx, lastMatch) = let
	        val (actIdx, action) = ruleToAction ruleIdx
		val actName = actName actIdx
		val lastMatch' = if hasREJECT action
				 then lastMatch
	...
	(* collect all valid transition symbols *)
	  ...
	  val errAct' = 
	        (case curMatch
		  of SOME j => let
		       val (actIdx, action) = ruleToAction j
		       in ML_App (actName actIdx, 
				  [ML_Var "strm", 
				   if hasREJECT action
				   then lastMatch
				   else ML_Var "yyNO_MATCH"])
    ...
    fun mkMachine actions (machine, k) = let
          val LO.Machine {label, rules, states} = machine
	  fun ruleToAction idx = let
	        val actIdx = #2 (Vector.sub (rules, idx))
		val action = Vector.sub (actions, actIdx)
		in
		  (actIdx, action)
		end
	  val machExp = ML_App (nameOf' 0, 
				[ML_RefGet (ML_Var "yystrm"), 
				 ML_Var "yyNO_MATCH"])
	  val states = List.foldr (mkState ruleToAction) machExp states
          in 
            ML_Fun ("yy" ^ label, [], states, k)
          end

    fun lexerHook spec strm = let
          val LO.Spec {actions, machines, ...} = spec
	  fun machMatch (LO.Machine {label, ...}) =
	        (ML_ConPat (label, []), ML_App ("yy" ^ label, []))
	  val innerExp = ML_Case (ML_RefGet (ML_Var "yyss"),
				  List.map machMatch machines)
	  val machineExp = List.foldr (mkMachine actions) innerExp machines
	  val lexerExp = Vector.foldri mkAction machineExp actions
    ...
    fun startStatesHook spec strm = let
	  ...
	  fun machName (LO.Machine {label, ...}) = label
	  val machNames = List.map machName machines
          in 
