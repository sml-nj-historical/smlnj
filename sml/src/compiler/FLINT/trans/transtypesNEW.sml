...
and toTyc d t = toTycGen tycTyc d t

and toTycGen tycTycFix d t = 
...
and toLtyGen x d (POLYty {tyfun=TYFUN{arity=0, body}, ...}) = toLtyGen x d body
  | toLtyGen x d (POLYty {tyfun=TYFUN{arity, body},...}) = 
      let val ks = LT.tkc_arg arity
       in LT.ltc_poly(ks, [toLtyGen x (DI.next d) body])
      end
  | toLtyGen tycTycFix d x = LT.ltc_tyc (toTycGen tycTycFix d x)

(****************************************************************************
 *               TRANSLATING ML MODULES INTO FLINT TYPES                    *
 ****************************************************************************)

fun specLty toLtySp (elements, entEnv, depth, compInfo) = 
  let fun g ([], entEnv, ltys) = rev ltys
        ...
        | g ((sym, spec)::rest, entEnv, ltys) =
              let val _ = debugmsg ">>specLtyElt"
                  ...
                  fun mapty t = toLtySp depth (transty t)
...
and strRlznLty toLtySp (sign, rlzn, depth, compInfo) = 
...
*)
        | g _ = strMetaLty toLtySp (sign, rlzn, depth, compInfo)

   in g(sign, rlzn)
  end

and fctRlznLty toLtySp (sign, rlzn, depth, compInfo) = 
  let fun g (sign, rlzn as {lambdaty = ref (SOME (lt, od)), ...}) = 
             LT.lt_adj(lt, od, depth)
        | g (sign as FSIG{paramsig, bodysig, ...},
             rlzn as {stamp, closure as CLOSURE{env,...}, lambdaty, ...}) = 
               let val {rlzn=argRlzn, tycpaths=tycpaths} = 
                   ...
                   val nd = DI.next depth
                   val paramLty = 
                     strMetaLty toLtySp (paramsig, argRlzn, nd, compInfo)
                   val ks = map tpsKnd tycpaths
                   val bodyRlzn = 
                     EV.evalApp(rlzn, argRlzn, nd, EPC.initContext,
                                IP.empty, compInfo)
                   val bodyLty = 
                     strRlznLty toLtySp (bodysig, bodyRlzn, nd, compInfo)
...
and strLtyGen toLtySp (str, depth, compInfo) = 
  let fun g (STR{rlzn={lambdaty=ref (SOME (lt, od)), ...}, ...}) = 
              LT.lt_adj(lt, od, depth)
        | g (STR{sign, rlzn as {lambdaty as ref NONE, ...}, ...}) = 
              let val lt = strRlznLty toLtySp (sign, rlzn, depth, compInfo)
               in (lambdaty := SOME(lt, depth); lt)
              end
...
and fctLtyGen toLtySp  (fct, depth, compInfo) = 
  let fun g (FCT{rlzn={lambdaty=ref(SOME (lt,od)),...}, ...}) = 
              LT.lt_adj(lt, od, depth)
        | g (FCT{sign, rlzn as {lambdaty as ref NONE, ...}, ...}) = 
              let val lt = fctRlznLty toLtySp (sign, rlzn, depth, compInfo)
               in (lambdaty := SOME(lt,depth); lt)
              end
...
fun genTT() = 
  let val m1 = ref (MIDict.mkDict())   (* modid (tycon) -> LT.tyc *)
      val m2 = ref (MIDict.mkDict())   (* modid (str/fct) -> LT.lty *)
      fun tycTycLook (t as (GENtyc _ | DEFtyc _), d) = 
            let val tid = MU.tycId t
             in (case MIDict.peek(!m1, tid)
                  of SOME (t', od) => LT.tc_adj(t', od, d)
                   | NONE => 
                       let val x = tycTyc (t, d)
                           val _ = (m1 := MIDict.insert(!m1, tid, (x, d)))
                        in x
                       end)
            end
        | tycTycLook x = tycTyc x

      val toTyc = toTycGen tycTycLook
      val toLty = toLtyGen tycTycLook
      val coreDict = (toTyc, toLty)
      val strLty = strLtyGen toLty
      val fctLty = fctLtyGen toLty
(*
