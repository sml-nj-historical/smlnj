...
functor BackEnd
  (structure MLTreeComp : MLTREECOMP    (* instruction selection *)
   ...
   structure Asm        : INSTRUCTION_EMITTER
      sharing InsnProps.I = Flowgraph.I = Asm.I = Rewrite.I = MLTreeComp.I = I
      sharing Asm.P = Flowgraph.P = MLTreeComp.T.PseudoOp

   val initialSpillOffset : int (* initial offset of the spill area in frame *)
   val spillAreaSize      : int (* max size of spill area *)

   val availRegs          : I.C.cell list  (* registers available for RA *)
   val availFRegs         : I.C.cell list  (* same for floating point *)
   val dedicatedRegs      : I.C.cell list  (* register not available for RA *)
   val dedicatedFRegs     : I.C.cell list  (* same for floating point *)
...
struct
   ...
   in
      structure GR = GetReg(val first=low val nRegs=high-low+1 
                            val available=availRegs)
       val dedicatedR = Array.array(high+1,false)
       val _ = app (fn r => Array.update(dedicatedR,r,true)) dedicatedRegs
   ...
   in
      structure FR = GetReg(val first=low val nRegs=high-low+1 
                            val available=availFRegs)
      val dedicatedF = Array.array(high+1,false)
      val _ = app (fn r => Array.update(dedicatedF,r,true)) dedicatedFRegs
   end
   ...
   fun newOffset offset = 
       if offset >= spillAreaSize then error "spill area too large"
   ...
   val KR = length availRegs
   val KF = length availFRegs
   ...
   fun compile cluster =
      (ra cluster;                (* just run register allocation *)
       Assembler.asmEmit cluster  (* and output the assembly *)
      ) 
