...
structure IntRedBlackMap :> ORD_MAP where type Key.ord_key = int =
  struct
    ...
    and 'a tree
      = E
      | T of (color * 'a tree * int * 'a * 'a tree)
   ...
   *)
    local
      datatype 'a zipper
	= TOP
	| LEFT of (color * int * 'a * 'a tree * 'a zipper)
	| RIGHT of (color * 'a tree * int * 'a * 'a zipper)
    in
   ...
   *)
    datatype 'a digit
      = ZERO
      | ONE of (int * 'a * 'a tree * 'a digit)
      | TWO of (int * 'a * 'a tree * int * 'a * 'a tree * 'a digit)
