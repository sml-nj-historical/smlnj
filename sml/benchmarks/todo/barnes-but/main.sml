...
functor Main (V : VECTOR) : sig
    ...
    val utestdata : int -> S.body list
	(* generate the uniform model data *)
  ...
  (* pick a random point on a sphere of specified radius. *)
    ...
    fun utestdata n = let
	  ...
	  fun mkBodies (0, cmr, cmv, l) = let
		...
		val r = 1.0 / sqrt (pow(xrand(0.0, mfrac), ~2.0/3.0) - 1.0)
		val pos = V.tabulate (fn _ => coeff*xrand(0.0, mfrac))
		...
		val v = ((sqrt 2.0) * vN()) / pow(1.0 + r*r, 0.25)
